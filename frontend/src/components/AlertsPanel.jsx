  ...
  return (
    <section className="flex flex-col rounded-xl border border-slate-800 bg-slate-950/70 p-4 shadow-sm shadow-slate-900/40">
      <div className="mb-3 flex items-center justify-between gap-2">
        <h2 className="text-sm font-semibold text-slate-100 sm:text-base">Anomaly Records</h2>
        <span className="rounded-full bg-slate-900 px-2 py-0.5 text-[11px] font-medium text-slate-300">
      ...
      <div className="space-y-3">
        {alerts.length === 0 ? (
          <div className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-[12px] text-slate-300">
            No anomalies detected. All resources are normal.
          </div>
        ) : (
          alerts.map((alert) => (
            <div
